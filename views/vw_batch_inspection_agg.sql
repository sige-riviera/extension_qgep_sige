...
CREATE OR REPLACE VIEW qgep_sige.vw_batch_inspection_agg
 AS
 SELECT row_number() OVER (ORDER BY vw_edit_collecting_pipe.usr_batch_inspection_id) AS row_number,
    vw_edit_collecting_pipe.usr_batch_inspection_id,
  ...
  WHERE vw_edit_collecting_pipe.usr_batch_inspection_id IS NOT NULL
  GROUP BY vw_edit_collecting_pipe.usr_batch_inspection_id;
