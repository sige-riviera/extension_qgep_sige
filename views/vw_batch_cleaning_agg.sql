...
CREATE OR REPLACE VIEW qgep_sige.vw_batch_cleaning_agg
 AS
 SELECT row_number() OVER (ORDER BY vw_edit_collecting_pipe.usr_batch_cleaning_id) AS row_number,
    vw_edit_collecting_pipe.usr_batch_cleaning_id,
    ...
    st_linemerge(st_multi(st_collect(vw_edit_collecting_pipe.progression_geometry))) AS batch_cleaning_geom
   FROM qgep_sige.vw_edit_collecting_pipe
  WHERE vw_edit_collecting_pipe.usr_batch_cleaning_id IS NOT NULL
  GROUP BY vw_edit_collecting_pipe.usr_batch_cleaning_id;
