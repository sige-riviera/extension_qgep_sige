-- FUNCTION: qgep_od.update_wastewater_structure_label(text, boolean)

-- DROP FUNCTION IF EXISTS qgep_od.update_wastewater_structure_label(text, boolean);
...
FROM (
  SELECT  ws_obj_id,
          ...
          array_to_string(array_agg(E'\nE' || idx || '=' || rpi_level ORDER BY idx), '', '') as input_label,
          array_to_string(array_agg(E'\nS' || idx || '=' || rpo_level ORDER BY idx), '', '') as output_label
  FROM (
...
ALTER FUNCTION qgep_od.update_wastewater_structure_label(text, boolean)
    OWNER TO postgres;
